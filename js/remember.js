...
window.onload = function () {
  ...
  var oRemember = document.getElementById("inputRememberPassword");
  //页面初始化时，如果帐号密码cookie存在则填充
  if (getCookie("inputEmail") && getCookie("inputPassword")) {
    oUser.value = getCookie("inputEmail");
  ...
  oForm.onsubmit = function () {
    if (oRemember.checked) {
      setCookie("inputEmail", oUser.value, 7); //保存帐号到cookie，有效期7天
